	...
	static class RequestHandler implements Runnable {
		...
		public void run() {
			try (DataInputStream dis = new DataInputStream(socket.getInputStream());
				...
				// MEHMET DEBUGGING
				String ownerId = "", requestId = null, clientId = "";
				...
				for (String part : parts) {
					...
					if (trimmed.startsWith("Request ID:"))
						...
						ownerId = trimmed.substring("Owner ID:".length()).trim();
					} else if (trimmed.startsWith("Client ID:")) {
						clientId = trimmed.substring("Client ID:".length()).trim();
				...
				if (requestId == null || requestId.isEmpty()) {
				    throw new IllegalStateException("requestId missing: " + data);
				}
				...
				if ("ACCEPTED".equals(decision)) {
					...
					if (ownerId != null && !ownerId.isEmpty()) {
						...
						db.insertRequest(ownerId, requestId, vehicleID, vehicleMake, vehicleModel, vehicleYear,
						...
						db.clientInsert(clientId, requestId, timestamp, jobDuration, jobDeadline

						);

						System.out.println("Decision: ACCEPTED — client data saved to log & database");
					}
